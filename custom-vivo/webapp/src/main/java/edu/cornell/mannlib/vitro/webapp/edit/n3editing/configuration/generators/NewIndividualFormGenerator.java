...
public class NewIndividualFormGenerator extends BaseEditConfigurationGenerator implements EditConfigurationGenerator {
  ...
  public void addFormSpecificData(EditConfigurationVTwo editConfiguration, VitroRequest vreq) {
    ...
    String isPersonType = isPersonType(vreq) ? "true" : "false";
    formSpecificData.put("isPersonType", isPersonType);

    //Put in whether or not data type
    //Doing this b/c unsure how freemarker will handle boolean value from JAVA
    String isDataType = isDataType(vreq) ? "true" : "false";
    formSpecificData.put("isDataType", isDataType);

    String typeOfNew = getTypeOfNew(vreq);
    formSpecificData.put("typeOfNew", typeOfNew);    
  ...
  public boolean isDataType(VitroRequest vreq) {
    WebappDaoFactory wdf = vreq.getWebappDaoFactory();
    Boolean isDataType = Boolean.FALSE;
    String typeOfNew = getTypeOfNew(vreq);
    if(typeOfNew.equals("http://www.w3.org/ns/dcat#Catalog")) {
      isDataType = Boolean.TRUE;
    } else if(typeOfNew.equals("http://www.w3.org/ns/dcat#Dataset")) {
      isDataType = Boolean.TRUE;
    } else {
